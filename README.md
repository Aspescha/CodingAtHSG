...
A data-driven tool to estimate the fair market value of any luxury watch — built as the final project for *Skills: Introduction to Programming* at the University of St. Gallen (HSG).

**Authors:** Jan Nemeth, Daniel, Timon, Julian, Philipp, Yannick Allmann
...
The luxury watch market is **opaque, fragmented, and emotion-driven**. Identical watches list at wildly different prices across dealers, and a private seller has almost no objective way to know what their piece is worth.
...
- **Buyers** who want a sanity check before paying a dealer's asking price.
...
That's the gap this project fills — a transparent, reproducible estimator that turns six data fields into a credible price.
...
The first cell of the notebook installs everything you need:

```bash
pip install pandas numpy matplotlib seaborn scikit-learn category_encoders xgboost shap
```
...
**Heads up on runtime:** The hyperparameter tuning step (Part 6) uses 5-fold cross-validation over 25 random parameter combinations and takes roughly **5–10 minutes** depending on your machine. Everything else runs in seconds.
...
We load the Kaggle dataset of luxury watch listings into a pandas DataFrame and inspect the first few rows. We also briefly explain the abbreviated column names (`ref`, `mvmt`, `casem`, `bracem`, `yop`, `cond`) so the reader can follow what each field represents.
...
This is the core modeling section, structured in three steps:
...
- The dataset is a single point-in-time snapshot. Real watch prices move with market conditions; for high-value pieces a fresh appraisal is always recommended.
